...
- **Two front-ends, one core:** a CLI (`npx ts7-compat-guard`) and a GitHub Action (`uses: Booyaka101/ts7-compat-guard@v1`).
...
Publish to npm as `ts7-compat-guard` (`npm publish`), then tag `v1` and push to a public GitHub repo so the Action is usable as `uses: Booyaka101/ts7-compat-guard@v1`. The single highest-signal launch is a short post to the open **"Add support for TypeScript 7"** framework threads (e.g. [`nestjs/nest-cli#3479`](https://github.com/nestjs/nest-cli/issues/3479), [`vercel/next.js#95633`](https://github.com/vercel/next.js/discussions/95633)) — the exact audience hitting this wall right now.
